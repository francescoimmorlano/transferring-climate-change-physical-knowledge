...
a = 16/reduction
b = 18/reduction
fig, axs = plt.subplots(len(short_scenarios_list), figsize=(a, b))
...
for scenario_short_idx, scenario_short in enumerate(short_scenarios_list):
    # BEST
    axs[scenario_short_idx].scatter(np.arange(start_year_training_tl_obs, end_year_training_tl_obs+1), warming_annual_BEST_data_means, s=1, label=f'BEST observational data', color='black', zorder=7)
    ...
    axs[scenario_short_idx].fill_between(np.arange(start_year_training_tl_obs, start_year_val), -2, ensemble_predictions_means[0,:(start_year_val-1)-start_year_training_tl_obs+1], color='red', alpha=0.07, zorder = 0, linewidth=linewidth)
    axs[scenario_short_idx].fill_between(np.arange(end_year_val+1, end_year_training_tl_obs+1), -2, ensemble_predictions_means[0,(end_year_val+1)-start_year_training_tl_obs : end_year_training_tl_obs-start_year_training_tl_obs], color='red', alpha=0.07, zorder = 0, linewidth=linewidth)
    # validation set shading
    axs[scenario_short_idx].fill_between(np.arange(start_year_val, end_year_val+1), -2, ensemble_predictions_means[0, start_year_val-start_year_training_tl_obs : end_year_val-start_year_training_tl_obs+1], color='grey', alpha=0.12, zorder = 0, linewidth=linewidth)
    # DNNs predictions ensemble
    axs[scenario_short_idx].plot(np.arange(start_year_training_tl_obs, end_year_test_tl_obs+1), ensemble_predictions_means[scenario_short_idx,:], linewidth=1, label=f'DNNs multi-model mean', color='#ff0000', zorder=6) #1d73b3 #ad4139
    # predictions 5-95% range uncertainty shading
    axs[scenario_short_idx].fill_between(np.arange(start_year_training_tl_obs, end_year_test_tl_obs+1), q05_predictions[scenario_short_idx,:], q95_predictions[scenario_short_idx,:], facecolor='#ff0000', zorder=3, alpha=0.3) #7EFDFF #ec6e6c
    # CMIP6 ensemble
    axs[scenario_short_idx].plot(np.arange(start_year_training_tl_obs, end_year_test_tl_obs+1), smooth_ensemble_warming_simulations_means[scenario_short_idx,:], linewidth=1, label=f'CMIP6 multi-model mean', color='#634444', zorder=5) #F56113 #c5534f #474343
    # CMIP6 5-95% range uncertainty shading
    axs[scenario_short_idx].fill_between(np.arange(start_year_training_tl_obs, end_year_test_tl_obs+1), q05_simulations[scenario_short_idx,:], q95_simulations[scenario_short_idx,:], facecolor='#9c5050', zorder=1, alpha=0.2) #FFD67E #f6cecc #bcbcbc
...
for scenario_short_idx, scenario_short in enumerate(short_scenarios_list):
    ...
    ymin, ymax = axs[scenario_short_idx].get_ylim()
    yticks = np.arange(np.floor(ymin), np.ceil(ymax) + 1, step=1)  # Generate ticks at every integer
    axs[scenario_short_idx].set_yticks(yticks)
    ...
    if years_to_thresholds_ensemble[scenario_short_idx,1] != 0:
        ...
        axs[scenario_short_idx].hlines(y=2, xmin=1979, xmax=year_to_2_threshold, color='#000000', zorder=9, linewidth=0.5)
        axs[scenario_short_idx].vlines(x=year_to_2_threshold, ymin=-1, ymax=ensemble_predictions_means[scenario_short_idx,year_to_2_threshold-1979], color= '#000000', zorder=9, linewidth=0.5)
    ...
    for spine in axs[scenario_short_idx].spines.values():
        spine.set_linewidth(linewidth)  # Thin the surrounding box
    axs[scenario_short_idx].tick_params(axis='both', width=linewidth, length=2)  # Thinner ticks with shorter length
    
    
    plt.xlim(left=1979)
    plt.sca(axs[scenario_short_idx])
    plt.yticks(fontsize=17*scale)
    plt.xticks(fontsize=17*scale)
    
    if scenario_short_idx == 0:
        axs[scenario_short_idx].legend(loc='upper left', prop={'size':14*scale})

fig.add_subplot(1, 1, 1, frame_on=False)

plt.tick_params(labelcolor="none", bottom=False, left=False)
plt.xlabel('Years', fontsize=20*scale) #, labelpad=30)
plt.ylabel('Global average warming [°C]\nBase period: 1850–1900', fontsize=22*scale, labelpad=-5)
plt.ylabel('Surface Air Temperature relative to 1850–1900 (°C)', fontsize=22*scale, labelpad=-5)
plt.savefig(f'Fig_2.pdf', dpi=300, bbox_inches='tight')
plt.close()